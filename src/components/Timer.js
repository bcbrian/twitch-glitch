...
export default function Timer({
  ...
  const { current: timerLength } = useRef(parseInt(timerLengthMil, 10));
  const { current: startTime } = useRef(Date.now());
  const { current: stopTimer } = useRef(startTime + timerLength);
  ...
  const tenths = Math.floor((time - milMinutes - milSeconds) / 100);
  return timerStopped
    ? "00:00:00"
